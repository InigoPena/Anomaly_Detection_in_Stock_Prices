...
from transformers import TransformerAutoencoder  # Update this if file name differs
...
def normalize_tensor_data(data):
    # Normalize each feature over all sequences and timesteps
...
def main():
    ...
    model_save_path = "../../../models/transformers/Failed_model"
    ...
    for file in company_files:
        ...
        data = df.values.reshape(-1, 30, 5).astype(np.float32)
        data = normalize_tensor_data(data)
        ...
        model_file = os.path.join(model_save_path, f"{company.lower()}_complexModelTrans.pth")
        ...
        plt.grid()
        plt.legend()
        plot_file = os.path.join(plot_save_path, f"lossConDropout_{company}.png")
